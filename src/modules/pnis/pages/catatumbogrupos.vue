<template>
  <v-container>
    <h1 class="mb-4">{{ $t('modules.pnis.catatumbogrupo') }}</h1>
    <v-card>
      <v-card-text class="pa-0">
        <exp-data-table
          uuid="table-users_pnis"
          :endpoint="`${endpoint}/catatumbogrupos/filterbysurvey/5`"
          :showHeader="false"
...
const headers: any[] = [
  ...
  { key: 'grupo_productores', title: "Nombre", width: "auto", align: "start",  searchable: true, sortable: false, },
  { key: 'cedula_representante', title: "Ident. Repres.", width: "auto", align: "start", searchable: true, sortable: false, },
  { key: 'representante', title: "Representante", width: "auto", align: "start", searchable: true, sortable: false, },
...
const getValidationItems = async () => {
  ...
  try {
    const response = await axios.get(
      `/api/1.0/core/validationregister/missing-validation-items/${identificationnumber.value}/5`
    );
...
const getItemsValidadosBase = async () => {
  try {
    const response = await axios.get(`/api/1.0/core/validationregister/items-validacion/5/`);
...
const modalValidados = async (item: any, tipo: string='si') => {
  try {
    ...
    const response = await axios.get(`/api/1.0/core/validationregister/filterbydocumentnumber/${item.cedula_representante}/5/${tipo}`);
...
const clickView = async (item: any) => {
  router.push(`/pnis/catatumbogrupos/open/${item.id}`);
};
