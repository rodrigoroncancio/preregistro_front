...
const getSurveyData = async () => {
  try {
    ...
    const responseFormpersona = await apiGet(`api/2.0/inscripciones/formpersona/7030/`);
    const responseAdjunto1 = await apiGet(`api/2.0/inscripciones/personaadjunto/129/`);
    const responseAdjunto2 = await apiGet(`api/2.0/inscripciones/personaadjunto/130/`);
    const responsePredio = await apiGet(`api/2.0/inscripciones/predio/38/`);
    const responseLinea = await apiGet(`api/2.0/inscripciones/personalinea/17/`);

    await fillOutForm(
        responsePersona.data,
        responseFormpersona.data,
        responseAdjunto1.data,
        responseAdjunto2.data,
        responsePredio.data,
        responseLinea.data
    );
...
const fillOutForm = async (responsePersona: any, responseFormpersona: any, responseAdjunto1: any, responseAdjunto2: any, responsePredio: any, responseLinea: any) => {
  try {
    ...
    survey.data = {
      ...
      tiene_coca: responseFormpersona.data.tiene_coca ?? 1,
